# Project Alpha 0.5.0 Database Reset

Version 0.5.0 deliberately has no in-place upgrade path from 0.4.x. Existing database contents are discarded. Project Alpha never removes a production volume automatically.

## Pilot Server

1. Record the current Compose project name and confirm which volume or TrueNAS dataset contains only MySQL data. Do not remove uploads, configuration, backups, or the persisted encryption key.
2. Stop the stack:

   ```bash
   docker compose down
   ```

3. Inspect the candidate volume before deletion:

   ```bash
   docker volume ls
   docker volume inspect <confirmed-project-alpha-db-volume>
   ```

4. Remove only the confirmed `pa_db_data` volume:

   ```bash
   docker volume rm <confirmed-project-alpha-db-volume>
   ```

   For a TrueNAS host-path installation, stop the app and delete/recreate only the configured database dataset through the TrueNAS storage UI.

5. Pull/build 0.5.0 and deploy:

   ```bash
   docker compose pull
   docker compose up -d
   docker compose ps -a
   docker compose logs migrate
   ```

6. Confirm `migrate` exited successfully, web and cron are healthy/running, and the logs contain no password hash.
7. Sign in using `admin@project-alpha.local` and the Compose `ADMIN_PASSWORD`; recreate the client and application settings.
8. Restart the stack and verify login, documents, cron, uploads, configuration, and backups still work.
9. Change `ADMIN_PASSWORD` in Compose, redeploy, and verify the new password works while the old one fails.

## Second Server

Reset the second server only after the pilot passes every check. Use the identical image tags and repeat the same volume inspection, database-only removal, deployment, login, restart, and administrator synchronization checks.

`docker compose down -v` is intentionally not used because it also removes non-database named volumes.
