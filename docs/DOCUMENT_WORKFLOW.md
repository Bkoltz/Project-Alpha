...
- `quote_auto_create_invoice` creates an unpaid regular invoice.
...
A valid quote link allows the client to approve or deny a pending quote. Approval creates a pending contract. A regular quote also creates an unpaid invoice; long-term and on-demand quotes wait for their contract-specific invoice workflow.
...
- Creating a regular contract directly creates a related unpaid invoice.
...
- Completing the contract sets `completed_at` and, if the newest related invoice has no due date, applies the configured net terms.
...
- Optional email-on-generation creates a public invoice link and records the notification to prevent duplicate sends.
...
- Auto-charge attempts for eligible recurring invoices
