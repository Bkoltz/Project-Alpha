...
final class PortalProjectionDeliveryConfigService
{
    ...
    public function saveProfile(PDO $pdo,int $profileId,array $input,int $actorId):void
    {
        $owns=!$pdo->inTransaction();try{if($owns)$pdo->beginTransaction();$profile=PortalProjectionService::lockProfileContract($pdo,$profileId);$credentials=$this->credentials($profile,false);
            $enabled=!empty($input['delivery_enabled']);$keyId=$this->keyId((string)($input['delivery_key_id']??($profile['delivery_key_id']??'')));$secret=trim((string)($input['delivery_secret']??''));$authHeaders=$this->authHeaders((string)($input['delivery_auth_headers_json']??''),$credentials['authHeaders']??[]);
            $currentKey=(string)($profile['delivery_key_id']??'');$rotating=$currentKey!==''&&!hash_equals($currentKey,$keyId);
            ...
            if($secret==='')$secret=$rotating?'':(string)($credentials['currentSecret']??'');if($secret!==''&&(strlen($secret)<32||strlen($secret)>1000))throw new DomainException('The signing secret must be 32 to 1000 characters.');
            ...
            if($rotating){if($secret==='')throw new DomainException('A new signing secret is required when rotating the key ID.');$previousKey=$currentKey;$previousSecret=(string)($credentials['currentSecret']??'');$overlap=max(1,min(168,(int)($input['delivery_overlap_hours']??48)));$previousUntil=(new DateTimeImmutable('now',new DateTimeZone('UTC')))->modify('+'.$overlap.' hours')->format('Y-m-d H:i:s.u');}
