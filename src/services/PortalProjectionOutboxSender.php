...
final class PortalProjectionOutboxSender
{
    ...
    private function profileAllows(array$profile,array$row):bool{if(empty($profile['delivery_enabled']))return false;if(!empty($row['is_revocation']))return true;if(empty($profile['enabled']))return false;return($row['route_type']??'')==='catalog'?!empty($profile['catalog_projection_enabled']):!empty($profile['portal_projection_enabled']);}
