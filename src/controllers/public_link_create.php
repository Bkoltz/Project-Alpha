...
try {
    ...
    // Ensure public_links table exists with expire_when_paid column
    try {
        ...
        // Add expire_when_paid column if it doesn't exist
        try {
    ...
    if ($forceNew) {
        $revokeSt = $pdo->prepare('UPDATE public_links SET revoked = 1 WHERE type = ? AND record_id = ? AND revoked = 0');
    ...
    if (!$forceNew) {
        $existingStmt = $pdo->prepare('
            ...
            FROM public_links 
            WHERE type = ? AND record_id = ? AND revoked = 0 
    ...
    // Insert new link
    $ins = $pdo->prepare('INSERT INTO public_links (type, record_id, token, expires_at, expire_when_paid) VALUES (?, ?, ?, ?, ?)');
