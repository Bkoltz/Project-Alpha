...
$id = (int)($_GET['id'] ?? 0);
if ($term === '' && $id <= 0) { echo json_encode([]); exit; }
...
$sql = "SELECT c.id, c.name, c.email, c.organization_id, c.state, c.postal_code, o.name as org_name, o.tax_exempt_file, o.state AS organization_state, o.postal_code AS organization_postal_code, COALESCE(NULLIF(o.postal_code, ''), NULLIF(c.postal_code, '')) AS preferred_tax_zip, COALESCE(NULLIF(o.state, ''), NULLIF(c.state, '')) AS preferred_tax_state FROM clients c LEFT JOIN organizations o ON c.organization_id = o.id {$whereSQL} ORDER BY c.name LIMIT 10";
