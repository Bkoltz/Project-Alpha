...
// Updated: uses unified contracts table, drops on_demand_invoices
...
try {
    // Fetch contract from unified table
    ...
    // Create invoice with unified contract reference
    ...
    $insertInvoice = $pdo->prepare('
        INSERT INTO invoices (
            ...
            subtotal, total, status, due_date, generated_at, created_at
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())
    ');
    ...
    $pdo->prepare('UPDATE contracts SET total_invoiced=?, invoice_count=?, last_invoice_date=? WHERE id=?')
