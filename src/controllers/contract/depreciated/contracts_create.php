...
if ($client_id <= 0) {
    ...
    if ($client_name !== '') {
        try {
            ...
            @error_log('[contracts_create] resolve by name failed: ' . $e->getMessage(), 0);
        }
    }
}
if ($client_id <= 0) {
...
try {
    ...
    try {
        ...
        @error_log('[contracts_create] project_next_code failed: ' . $e->getMessage(), 0);
    }
    ...
    foreach ($items as $it) {
        $ins->execute([$co_id, $it['i'], $it['d'], $it['q'], $it['p'], $it['t']]);
    }
    ...
    foreach ($items as $it) {
        $ii->execute([$invoice_id, $it['i'], $it['d'], $it['q'], $it['p'], $it['t']]);
    }
