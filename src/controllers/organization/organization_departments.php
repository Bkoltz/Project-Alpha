...
try {
    ...
    if ($action === 'save_department') {
        ...
        $resolverMode = (string)($_POST['resolver_mode'] ?? 'manual_only');
        ...
        $stmt = $pdo->prepare('
            INSERT IGNORE INTO organization_department_contacts (department_id, client_id, role)
            VALUES (?, ?, "contact")
        ');
        $stmt->execute([$departmentId, $clientId]);
