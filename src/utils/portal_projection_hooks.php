 ...
 */
function portal_projection_mutate(PDO $pdo,array|callable $beforeScopes,callable $mutation,callable $afterScopes):mixed
{
    $owns=!$pdo->inTransaction();try{if($owns)$pdo->beginTransaction();$lockedBeforeScopes=is_callable($beforeScopes)?$beforeScopes():$beforeScopes;$result=$mutation();$scopes=array_merge($lockedBeforeScopes,$afterScopes());(new PortalProjectionMutationService())->afterMutation($pdo,$scopes);if($owns)$pdo->commit();return$result;}catch(Throwable$error){if($owns&&$pdo->inTransaction())$pdo->rollBack();throw$error;}
}
