...
class ContractListRepository extends BaseListRepository
{
    ...
    private const DOCUMENT_TYPE_STATEMENTS = [
        DocumentType::REGULAR->value => 'SELECT co.id, co.doc_number, co.project_code, co.status, co.total, co.deposit_type, co.deposit_value, co.deposit_paid, co.signed_pdf_path, c.name client_name, c.id AS client_id FROM contracts co JOIN clients c ON c.id=co.client_id',
        DocumentType::ON_DEMAND->value => 'SELECT co.id, co.doc_number, co.project_code, co.status, co.start_date, co.end_date, co.billing_interval_count, co.billing_interval_unit, co.price_per_invoice, co.total_invoiced, co.invoice_count, co.last_invoice_date, c.name client_name, c.id AS client_id FROM on_demand_contracts co LEFT JOIN clients c ON c.id=co.client_id',
        DocumentType::LONG_TERM->value => 'SELECT co.id, co.doc_number, co.project_code, co.status, co.total, co.deposit_type, co.deposit_value, co.deposit_paid, co.start_date, co.end_date, co.billing_interval_count, co.billing_interval_unit, co.pricing_type, co.price_per_invoice, co.total_invoiced, co.next_invoice_date, c.name client_name, c.id AS client_id FROM long_term_contracts co LEFT JOIN clients c ON c.id=co.client_id',
    ];
