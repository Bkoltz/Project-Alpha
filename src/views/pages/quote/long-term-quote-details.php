...
$id = (int)($_GET['id'] ?? 0);
$q = $pdo->prepare('SELECT q.*, cl.name client_name, cl.organization client_org, cl.email client_email, cl.phone client_phone, cl.address_line1, cl.address_line2, cl.city, cl.state, cl.postal, cl.country FROM quotes q JOIN clients cl ON cl.id=q.client_id WHERE q.id=? AND q.is_long_term=1');
$q->execute([$id]);
