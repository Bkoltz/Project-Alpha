...
// Valid tabs
$validTabs = ['system', 'terms', 'billing', 'taxes', 'workflow', 'documents', 'notifications', 'links', 'item-library', 'backup', 'logs', 'permissions', 'alphaledger'];
...
<section>
  ...
  <div style="display:grid;grid-template-columns:220px 1fr;gap:16px;margin-top:12px">
    <aside style="border:1px solid #eee;border-radius:8px;overflow:hidden;background:#fff">
      ...
      <a href="/?page=settings&tab=workflow" style="display:block;padding:10px 12px;border-bottom:1px solid #eee;<?php echo $tab === 'workflow' ? 'background:#f8fafc;font-weight:600' : ''; ?>">Workflow</a>
      ...
      <a href="/?page=settings&tab=alphaledger" style="display:block;padding:10px 12px;border-bottom:1px solid #eee;<?php echo $tab === 'alphaledger' ? 'background:#f8fafc;font-weight:600' : ''; ?>">AlphaLedger</a>
    ...
    <div>
      <?php if ($tab === 'taxes' || $tab === 'item-library' || $tab === 'documents' || $tab === 'links' || $tab === 'backup' || $tab === 'permissions' || $tab === 'logs' || $tab === 'alphaledger'): ?>
        <?php
