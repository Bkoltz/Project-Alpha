...
<section>
  ...
  <form method="post" action="/?page=clients-create" style="display:grid;gap:12px;max-width:520px">
    ...
    <label>
      <div>Name</div>
      <input required type="text" name="name" placeholder="First Last" class="input">
    </label>
    <label>
      <div>Email</div>
    ...
    <label>
      <div>Phone</div>
      <input type="text" name="phone" placeholder="(555) 123-4567" class="input">
    </label>
    <label style="position:relative">
      <div>Organization</div>
      <input type="text" id="orgInput" placeholder="Type to search organizations..." autocomplete="off" class="input">
    ...
    <fieldset style="border:1px solid #eee;border-radius:8px;padding:12px">
      ...
      <label>
        <div>Address line 1</div><input name="address_line1" class="input">
      </label>
      <label>
        <div>Address line 2</div><input name="address_line2" class="input">
      </label>
      <div style="display:grid;gap:8px;grid-template-columns:1fr 1fr 1fr">
        <label>
          <div>City</div><input name="city" class="input">
        </label>
        <label>
          <div>State</div><input name="state" value="<?php echo htmlspecialchars($appConfig['primary_state'] ?? ''); ?>" class="input">
        </label>
        <label>
          <div>Postal (zip)</div><input name="postal" class="input">
        </label>
      </div>
    </fieldset>
    <label>
      <div>Notes</div>
      <textarea name="notes" rows="3" placeholder="Internal notes" class="input"></textarea>
    </label>
  ...
  <div id="createOrgModal" style="display:none;position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,0.5);z-index:100;align-items:center;justify-content:center;flex-direction:column;-webkit-align-items:center;-webkit-justify-content:center">
    <div style="background:#fff;padding:24px;border-radius:12px;max-width:400px;box-shadow:0 20px 25px rgba(0,0,0,0.15)">
      ...
      <form id="createOrgForm" class="grid">
        <input type="hidden" id="createOrgCsrf" name="csrf" value="">
        <label>
          <div style="font-weight:500;margin-bottom:4px">Organization Name</div>
          <input type="text" id="createOrgNameInput" name="name" required placeholder="Organization name" class="input">
        </label>
