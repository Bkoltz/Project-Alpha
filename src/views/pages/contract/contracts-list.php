...
$sql .= " ORDER BY co.created_at DESC LIMIT $per OFFSET $offset";
$st = $pdo->prepare($sql);
$st->execute($p);
$rows = $st->fetchAll();
...
<section>
        ...
        <?php foreach ($rows as $r): ?>
          ...
          <tr style="border-top:1px solid #f3f4f6;<?php echo $rowStyle; ?>">
            ...
            <td style="padding:10px;display:flex;flex-wrap:wrap;gap:8px;align-items:center">
              ...
              <?php $cst = strtolower((string)$r['status']);
              if (!in_array($cst, ['denied', 'cancelled', 'void'], true)): ?>
  ...
  $qs = $_GET;
  unset($qs['p']);
  $base = '/?' . http_build_query($qs + ['page' => 'contract/contracts-list', 'per_page' => $per]);
  ?>
