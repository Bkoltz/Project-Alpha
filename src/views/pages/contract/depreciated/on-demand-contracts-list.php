...
if ($project_code !== '') {
  $where[] = 'odc.project_code LIKE ?';
  $p[] = $project_code . '%';
}
if ($doc_no > 0) {
  $where[] = 'odc.doc_number=?';
  $p[] = $doc_no;
}
if ($min_price !== null) {
  $where[] = 'odc.price_per_invoice>=?';
  $p[] = $min_price;
}
if ($max_price !== null) {
  $where[] = 'odc.price_per_invoice<=?';
...
<section>
  ...
  $qs = $_GET;
  unset($qs['p']);
  $base = '/?' . http_build_query($qs + ['page' => 'contract/on-demand-contracts-list', 'per_page' => $per]);
  ?>
