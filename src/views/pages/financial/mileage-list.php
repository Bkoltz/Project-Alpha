...
$stmt=$pdo->prepare('SELECT m.*,
  ...
  COALESCE(NULLIF(TRIM(CONCAT_WS(" ",ep.first_name,ep.last_name)),""),NULLIF(u.username,""),u.email,"Former user") driver_name
  FROM mileage_logs m LEFT JOIN users u ON u.id=COALESCE(m.traveler_user_id,m.user_id) LEFT JOIN employee_profiles ep ON ep.user_id=u.id
  LEFT JOIN mileage_charge_allocations a ON a.mileage_log_id=m.id LEFT JOIN clients c ON c.id=a.client_id
...
if($canViewAllDrivers){
  $driverStmt=$pdo->prepare('SELECT u.id,COALESCE(NULLIF(TRIM(CONCAT_WS(" ",ep.first_name,ep.last_name)),""),NULLIF(u.username,""),u.email) name FROM users u LEFT JOIN employee_profiles ep ON ep.user_id=u.id WHERE u.deleted_at IS NULL AND (u.id=? OR EXISTS (SELECT 1 FROM mileage_logs md WHERE COALESCE(md.traveler_user_id,md.user_id)=u.id)) ORDER BY name');
