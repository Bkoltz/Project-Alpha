...
$canManageAllTime = WorkforceSettings::canManageAllTime($pdo, $userId);
$approvalPolicy = new TimeApprovalPolicy($pdo);
$canReviewTime = $approvalPolicy->canAccessQueue($userId);
$reviewQueue = new TimeReviewQueueService($pdo, $approvalPolicy);
$manageWorkforce = $canManageAllTime || $canReviewTime;
$canManageAccounts = $isAdmin || user_can($pdo, $userId, 'users.manage', 0);
...
$canViewPay = $canManagePay
    || user_can($pdo, $userId, 'employee_pay.self', 0)
    || user_can($pdo, $userId, 'workforce.statements.self', 0);
...
if ($manageWorkforce) {
    ...
    if (!$canManageAllTime) {
        $employeeRows = array_values(array_filter(
            $employeeRows,
            static fn(array $employee): bool => $approvalPolicy->canReviewWorker($userId, (int)$employee['id'])
        ));
    }
    foreach ($employeeRows as &$employee) {
        $employee['review_count'] = $reviewCounts[(int)$employee['id']] ?? 0;
    }
    unset($employee);
    if ($canManagePay) {
        $pendingPay = array_sum(array_map(
            static fn(array $employee): float => (float)$employee['pending_pay'],
            $employeeRows
        ));
    }
    $activeEmployees = count(array_filter(
        $employeeRows,
        static fn(array $employee): bool => $employee['employment_status'] === 'active'
    ));
    $runningNow = $canManageAllTime
        ? (int)$pdo->query("SELECT COUNT(*) FROM work_time_entries WHERE status='running' AND end_time IS NULL")->fetchColumn()
        : count(array_filter($employeeRows, static fn(array $employee): bool => !empty($employee['running'])));
} else {
    $stmt = $pdo->prepare(
  ...
  <?php if ($manageWorkforce): ?>
    ...
    <div class="workforce-overview-grid">
      <article class="card workforce-card workforce-card--table">
        ...
        <div class="pa-table-wrap"><table class="pa-table workforce-table"><thead><tr><th>Employee</th><th>Last 30 days</th><?php if ($canReviewTime): ?><th>Review</th><?php endif; ?><?php if ($canManagePay): ?><th>Pending pay</th><?php endif; ?><?php if ($canManageAccounts): ?><th></th><?php endif; ?></tr></thead><tbody>
          <?php foreach ($employeeRows as $employee): ?>
            <tr>
              ...
              <?php if ($canReviewTime): ?><td><?= number_format((int)$employee['review_count']) ?> awaiting</td><?php endif; ?>
              <?php if ($canManagePay): ?><td><?= $h($money((float)$employee['pending_pay'])) ?></td><?php endif; ?>
              <?php if ($canManageAccounts): ?><td class="text-right"><a class="btn btn-sm" href="/?page=account-edit&amp;id=<?= (int)$employee['id'] ?>">Account &amp; assignments</a></td><?php endif; ?>
            </tr>
          <?php endforeach; ?>
          <?php if (!$employeeRows): ?><tr><td colspan="<?= 2 + ($canReviewTime ? 1 : 0) + ($canManagePay ? 1 : 0) + ($canManageAccounts ? 1 : 0) ?>" class="workforce-empty">No employee accounts are available in your workforce scope.</td></tr><?php endif; ?>
        </tbody></table></div>
      </article>
