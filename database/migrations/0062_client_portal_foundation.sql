-- Migration 0062: Provider-neutral, default-off client portal foundation.
-- Identity-provider enrollment, routes, invitations, and email remain disabled.

SET @portal_column_sql = IF(
    (SELECT COUNT(*) FROM information_schema.columns WHERE table_schema = DATABASE() AND table_name = 'organizations' AND column_name = 'public_id') = 0,
    'ALTER TABLE organizations ADD COLUMN public_id CHAR(32) CHARACTER SET ascii COLLATE ascii_bin NULL AFTER id',
    'SELECT 1'
);
PREPARE portal_column_stmt FROM @portal_column_sql;
EXECUTE portal_column_stmt;
DEALLOCATE PREPARE portal_column_stmt;

SET @portal_column_sql = IF(
    (SELECT COUNT(*) FROM information_schema.columns WHERE table_schema = DATABASE() AND table_name = 'clients' AND column_name = 'public_id') = 0,
    'ALTER TABLE clients ADD COLUMN public_id CHAR(32) CHARACTER SET ascii COLLATE ascii_bin NULL AFTER id',
    'SELECT 1'
);
PREPARE portal_column_stmt FROM @portal_column_sql;
EXECUTE portal_column_stmt;
...
SET @portal_column_sql = IF(
    (SELECT COUNT(*) FROM information_schema.columns WHERE table_schema = DATABASE() AND table_name = 'projects' AND column_name = 'public_id') = 0,
    'ALTER TABLE projects ADD COLUMN public_id CHAR(32) CHARACTER SET ascii COLLATE ascii_bin NULL AFTER id',
    'SELECT 1'
);
PREPARE portal_column_stmt FROM @portal_column_sql;
EXECUTE portal_column_stmt;
...
SET @portal_index_sql = IF(
    (SELECT COUNT(*) FROM information_schema.statistics WHERE table_schema = DATABASE() AND table_name = 'organizations' AND index_name = 'uq_organizations_public_id') = 0,
    'ALTER TABLE organizations ADD UNIQUE KEY uq_organizations_public_id (public_id)',
    'SELECT 1'
);
PREPARE portal_index_stmt FROM @portal_index_sql;
EXECUTE portal_index_stmt;
DEALLOCATE PREPARE portal_index_stmt;

SET @portal_index_sql = IF(
    (SELECT COUNT(*) FROM information_schema.statistics WHERE table_schema = DATABASE() AND table_name = 'clients' AND index_name = 'uq_clients_public_id') = 0,
    'ALTER TABLE clients ADD UNIQUE KEY uq_clients_public_id (public_id)',
    'SELECT 1'
);
PREPARE portal_index_stmt FROM @portal_index_sql;
EXECUTE portal_index_stmt;
...
SET @portal_index_sql = IF(
    (SELECT COUNT(*) FROM information_schema.statistics WHERE table_schema = DATABASE() AND table_name = 'projects' AND index_name = 'uq_projects_public_id') = 0,
...
-- Never overwrite an operator's existing choice when a migration is retried.
